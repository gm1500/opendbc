...
class CarControllerParams:
  ...
  def __init__(self, CP):
    ...
    if CP.carFingerprint in (CAMERA_ACC_CAR | SDGM_CAR):
      self.MAX_GAS = 3350.0 #as measured from stock allowed
      ...
      self.MAX_GAS = 2250.0  # Safety limit, not ACC max. Stock ACC >2042 from standstill.
...
class CAR(Platforms):
  ...
  GMC_ACADIA = GMASCMPlatformConfig(
    ...
    GMCarSpecs(mass=1975, wheelbase=2.86, steerRatio=14.4, centerToFrontRatio=0.4, wheelRadius=0.3835),
  )
  ...
  CHEVROLET_SILVERADO = GMPlatformConfig(
    ...
    GMCarSpecs(mass=2450, wheelbase=3.74, steerRatio=17.3, tireStiffnessFactor=1.0, wheelRadius=0.42, centerToFrontRatio=0.75),
  )
